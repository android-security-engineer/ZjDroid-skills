...
### 加固 vs ZjDroid：两个维度的对抗

```mermaid
flowchart TD
    subgraph FS["文件层加固（壳的防线）"]
        F1["APK 内 classes.dex<br/>（加密/壳代码）"] --> F2["文件层反编译"]
        F2 --> F3["❌ 只拿到壳<br/>真实 DEX 不可见"]
    end

    subgraph MS["内存层（ZjDroid 的战场）"]
        M1["App 启动"] --> M2["壳代码在内存解密真实 DEX"]
        M2 --> M3["明文 DEX 短暂存在于内存"]
        M3 --> M4["ZjDroid hook openDexFileNative"]
        M4 --> M5["捕获 mCookie 内存指针"]
        M5 --> M6["JNI 直读 Dalvik 内存"]
        M6 --> M7["✅ 导出明文 DEX"]
    end

    F3 -.->|"文件加固无效<br/>换维度取内存"| M4
```
